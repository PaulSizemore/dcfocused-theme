...
// Setting Vars 
$postID=get_the_id(); $postTime=get_the_time( 'F jS, Y' );$postTitle=get_the_title();  $postPerma=get_permalink(); 
...
$postAuthID=get_post( )->post_author;  // Returns a INT 
// ".the_author_posts_link() ."

$postAuthName=get_the_author(); 
// Content 
$rell=$rell."<div class=\"recentPostBox\" onmouseover=\"document.getElementById('textBlock_".$postID."').style.display = 'block';  \"  onmouseout=\"document.getElementById('textBlock_".$postID."').style.display = 'none';\">\n";
					$rell=$rell." \n	<div class=\"recentPosts_Text\"  id=\"textBlock_".$postID."\" >";
											$rell=$rell."<div class=\"recentPosts_TextTitleBox\">";
																...
																$rell=$rell."<div  class=\"recentPosts_author\">By <a   style=\"color:#FFFFFF;\"  href=\"".get_author_posts_url( get_the_author_meta('ID') ).get_the_author_meta('display_name')."?theme=Test-theme\">By ".$postAuthName."</a></div>";
																$rell=$rell." <div class=\"recentPosts_entry\">".$my_excerpt."</div>\n\n";
											$rell=$rell. "</div> <!-- recentPosts_TextTitleBox -->";	
					$rell=$rell. "</div> <!-- recentPosts_Text -->";	
					$rell=$rell."<div class=\"recentPosts_image\"  id=\"imageBlock_".$postID."\"   onmouseover=\"document.getElementById('textBlock_".$postID."').style.display = 'block';\" onmouseout=\"document.getElementById('textBlock_".$postID."').style.display = 'none';\"    style= \"background: url('".$postThumbURL."'); background-size: cover;\"></div>";
...
add_shortcode( 'tiled_blog_roll', 'tiled_blog_roll' );


// Add Shortcode
// [dcf_social_icons] returns the list of DC Focused Social Icons 
function dcfsocialicons_function() {
	$iconsDirectory = get_template_directory_uri()."/icons/";  
	$DCFsocial="<div style=\"text-align: right; padding-top: 30px; \">";
	$DCFsocial=$DCFsocial." <A href=\"https://www.facebook.com/dcfocused202/\" target=\"_blank\" style=\"padding-right:15px;\"><img src=\""; 
	$DCFsocial=$DCFsocial.$iconsDirectory;
	$DCFsocial=$DCFsocial."facebook.png\" alt=\"Connect with DC Focused on Facebook\" height=\"20\" width=\"20\"></A>";
	
	 $DCFsocial=$DCFsocial." <A href=\"https://www.instagram.com/dcfocused/\" target=\"_blank\" style=\"padding-right:15px;\"><img src=\""; 
	$DCFsocial=$DCFsocial.$iconsDirectory;
	$DCFsocial=$DCFsocial."instagram.png\" alt=\"Connect with DC Focused on Instagram\" height=\"20\" width=\"20\"></A>";
	
	 $DCFsocial=$DCFsocial." <A href=\"https://twitter.com/dcfocused/\" target=\"_blank\" style=\"padding-right:15px;\"><img src=\""; 
	$DCFsocial=$DCFsocial.$iconsDirectory;
	$DCFsocial=$DCFsocial."twitter.png\" alt=\"Connect with DC Focused on Twitter\" height=\"20\" width=\"20\"></A>";
	$DCFsocial=$DCFsocial."</div>";
	return $DCFsocial;}
	
add_shortcode( 'dcf_social_icons', 'dcfsocialicons_function' );


/* Latest posts shortcode */  /* Latest posts shortcode */  /* Latest posts shortcode */  /* Latest posts shortcode */
/* Edit November 2016 Paul Sizemore */ 
/* Latest posts shortcode */  /* Latest posts shortcode */  /* Latest posts shortcode */  /* Latest posts shortcode */

if (!function_exists('latest_post2')) {
function latest_post2($atts, $content = null) {	$blog_hide_comments = "";	if (isset($qode_options['blog_hide_comments'])) {	$blog_hide_comments = $qode_options['blog_hide_comments'];	}
$qode_like = "on";	if (isset($qode_options['qode_like'])) {	$qode_like = $qode_options['qode_like'];	}
$args = array(	"type"       			=> "boxes",	"number_of_posts"       => "",	"number_of_columns"     => "",	"number_of_rows"        => "1",		"rows"                  => "",	"order_by"              => "",	"order"                 => "",	"category"              => "",	"text_length"           => "",	"title_tag"             => "h4",
	"display_category"    	=> "1",	"display_date"          => "1",	"display_author"		=> "1",	"background_color"      => ""	);
...
$q = new WP_Query(array(	'orderby' => $order_by,	'order' => $order,	'posts_per_page' => $posts_number,	'category_name' => $category	));
//get number of columns class for boxes type
$columns_number = "";
if($type == 'boxes') {
			switch($number_of_columns) {
				case 1:	$columns_number = 'one_column';		break;
				case 2:	$columns_number = 'two_columns';	break;
				case 3:	$columns_number = 'three_columns';	break;
				case 4:	$columns_number = 'four_columns';				break;			default:		break;	}	}

//get number of rows class for boxes type
		$rows_number = "";
		if($type == 'boxes') {
			switch($number_of_rows) {
				case 1:	$rows_number = 'one_row';			break;
				case 2:	$rows_number = 'two_rows';			break;
				case 3:	$rows_number = 'three_rows';			break;
				case 4:	$rows_number = 'four_rows';		break;
				case 5:	$rows_number = 'five_rows';		break;		default:		break;	}	}

		$html = "";
		$html .= "<div class='latest_post_holder $type $columns_number $rows_number'    <!--  style='width:82% !important;--> '>";
		$html .= "<ul>";

		while ($q->have_posts()) : $q->the_post();	$li_classes = "";$box_style  = "";
			...
			if($background_color != "" && $type == "boxes"){
				if($background_color == "transparent" || $background_color == "rgba(0,0,0,0.01)"){
					...
					$box_style = "style='background-color: transparent; padding-top: 15px; '";
				}
			}
			...
			$html .= '<li class="clearfix">';
								...
								$html .= '<div class="latest_post_categories post_info_section" style="text-align: center !important; width:96%" >';
									foreach ($cat as $categ) {
										$html .= '<a href="' . get_category_link($categ->term_id) . '" style="margin:auto; text-transform: uppercase !important; color:#D0722D; font-size:120% !important; font-weight: bold;" >' . $categ->cat_name . ' </a> ';
									}
								$html .= '</div>'; //close span.latest_post_categories
							}
							
							
																																					 // $imageElement=get_the_post_thumbnail(get_the_ID(), 'portfolio-default');
																																					  $imageElement=get_the_post_thumbnail(get_the_ID(), array( 440, 440) ); // New size added 
				if($type == "boxes") {
				// Output of the image with the overlay / hover 
					$html .= '<div class="boxes_image">';
						$html .= '<a href="'.get_permalink().'">'.$imageElement.'<span class="latest_post_overlay" style="width:465px !important; ">';
						// Insert the content for the overlay 
				...
				$html .= '<div class="latest_post" '.$box_style.'>';
					if($type == "image_in_box") {
						$html .= '<div class="latest_post_image clearfix">';
																																							// $imageElement=get_the_post_thumbnail(get_the_ID(), 'latest_post_small_image');
																																							 $imageElement=get_the_post_thumbnail(get_the_ID(), array( 450, 450) );
							$html .= '<a href="'.get_permalink().'">'.$imageElement.'</a>';
						$html .= '</div>';
					}
	...
	add_shortcode('latest_post2', 'latest_post2');
}
/* Latest posts shortcode */  /* Latest posts shortcode */  /* Latest posts shortcode */  /* Latest posts shortcode */
/* Latest posts shortcode */  /* Latest posts shortcode */  /* Latest posts shortcode */  /* Latest posts shortcode */
/* Latest posts shortcode */  /* Latest posts shortcode */  /* Latest posts shortcode */  /* Latest posts shortcode */
...
// Add Shortcode
// [tilepost tilepostSlug="r-kellys-the-buffet-tour-shows-off-his-diverse-musical-ouevre-essay"] 
function tilepost_shortcode( $atts ) { $html=""; 
																		 // Attributes
																		 extract( shortcode_atts( array(  'tilepostslugleft' => 'DC Focused',  'tilepostslugright' => 'DC Focused',), $atts ) );
																		 $html .= "\n\n\n\n<!-- tilepostslugleft: ".$tilepostslugleft." -->\n\n";
																		 $html .= "\n\n\n\n<!-- tilepostslugright: ".$tilepostslugright." -->\n\n";
																		 
																		 $html=$html. "<div class=\"tilePostsContainer\"> <!-- postTilesContainer-->";
// LEFT TILE - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
// WP_Query arguments FOR LEFT Tile 
$args = array(
	'name'                   => $tilepostslugleft,
	'post_type'              => array( 'photo_essay', 'page', 'post' ),
...
$my_posts = new WP_Query($args);
			 $tilePostID = get_the_ID();    
			 $postTime=get_the_time( 'F jS, Y' );$postTitle=get_the_title();  
			// $my_excerpt = get_the_excerpt($tilePostID); 

		  while ( $my_posts->have_posts() ) {		
		  $my_posts->the_post();		
		  $postPerma=get_permalink(); 
		  // style='float:left; '
		  $html=$html. "<div class=\"tilepostcontainerleft\" >\n" ;  // Tile Post Container 
																										// Debug Info 
																										$html=$html. "<!-- ".$postTime." -->\n" ; 
																										$html=$html. "<!-- ".$tilePostID."-->\n";	
																									   //$html=$html. " ".$my_excerpt."<BR>\n" ; 
																									   
																										 // Tile Post Category  
																												$cat = get_the_category();
																												$html .= "<div class=\"tilePostCat\" style=\"margin-bottom:5px;\">";  foreach ($cat as $categ) { 
																												$html .= " ". $categ->cat_name . " ";	}
																												$html .= "</div>  <!-- // tilePostCat -->"; //close
																												
																										 	// Image
																												if ( has_post_thumbnail() ) {
																												$imageElement=	get_the_post_thumbnail( $titlepostID, array(450,450));
																												$html.="<div class=\"tilePostImg\" ><a href=\"".$postPerma."\">".$imageElement."</a></div>\n"; 
																												}  else {  $html.="<div class=\".tilePostError\" style=\"width:450px; height:450px; color:gray; font-size:150%; \">There is no Featured Image Defined.</div>\n";  } 																												
																												
																												// Title 
																												$htmlTitle=get_the_title($display=false);
																												$html=$html. "<div class=\"tilePostTitle\"><a  href=\"".$postPerma."\">".$htmlTitle."</a></div>\n";

$html=$html. "</div><!-- tilePostContainer --> \n" ;  // Tile Post Container 
}
...
// RIGHT TILE - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
// WP_Query arguments FOR RIGHT Tile 
$args = array(
	'name'                   => $tilepostslugright,
	'post_type'              => array( 'photo_essay', 'page', 'post' ),
	'post_status'            => array( 'publish', 'draft' ),
	'posts_per_page'         => '1',
	'orderby'                => 'modified',
...
$my_posts = new WP_Query($args);
		  ...
		  $postPerma=get_permalink(); 
		  // style='float:right; '> 
		  $html=$html. "<div class=\"tilepostcontainerright\" >\n" ;  // Tile Post Container 
																										// Debug Info 
																										$html=$html. "<!-- ".$postTime." -->\n" ; 
																										$html=$html. "<!-- ".$tilePostID."-->\n";	
																									   //$html=$html. " ".$my_excerpt."<BR>\n" ; 
																										 ...
																										 // Tile Post Category  
																										 	...
																										 	// Image
																												...
																												$imageElement=	get_the_post_thumbnail( $titlepostID, array(450,450));
																												$html.="<div class=\"tilePostImg\" ><a href=\"".$postPerma."\">".$imageElement."</a></div>\n"; 
																												}  else {  $html.="<div class=\".tilePostError\">There is no Featured Image Defined.</div>\n";  } 																												
																												
																												// Title 
																												$htmlTitle=get_the_title($display=false);
																												$html=$html. "<div class=\"tilePostTitle\">
																												<a class=\"tilePostTitleA\" href=\"".$postPerma."\">".$htmlTitle."</a></div>\n";
...
$html=$html. "</div> <!-- postTilesContainer-->";
return  $html; 

}
add_shortcode( 'tilepost', 'tilepost_shortcode' );
//  $imageElement=get_the_post_thumbnail(get_the_ID(), array( 440, 440) ); // New size added 

// Add Shortcode for the custom divider - - - - - - - -  
function divider_shortcode( $atts ) {
// Attributes
extract( shortcode_atts( array(  'text' => 'DC Focused',), $atts ) );
	return "<div class=\"separator_container\"><div class=\"separator_rule\"></div><div class=\"separator_content\">".$text."</div><div class=\"separator_rule\"></div></div><div style=\"clear:both;\"></div>";
}
add_shortcode( 'dcfdivider', 'divider_shortcode' );